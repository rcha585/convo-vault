...
This is the v0.7.22 local backend rendering direction: the extension keeps the
...
As of v0.7.14, the fixture regression flow also checks multilingual and
mixed-direction output across the HTML renderer, Markdown endpoint, and JSON
data sidecar. The UI remains English, but exported conversation text should
...
possible and records a clear degraded state when content cannot be represented
inside a static PDF.
...
The current `0.7.22` codebase can use Microsoft Edge as an independent backend
...
- Preserves Markdown structure, tables, links, blockquotes, lists, code blocks,
  ...
  numbers.
- Uses neutral document language metadata and broader font fallback for
  multilingual Unicode exports, including CJK, Spanish, Italian, Korean, emoji,
  and mixed-language conversations.
- Renders formula blocks/inline formulas as static PDF formula panels with
  common superscript, subscript, fraction, square-root, and symbol handling.
- Renders Mermaid flowchart, sequenceDiagram, erDiagram, and stateDiagram as
  static SVG where possible. Unsupported Mermaid diagram types are preserved as
  source with an explicit degraded note.
- Renders simple `chart` fences from JSON or CSV-like data as static bar/line
  SVG charts for PDF output.
...
- Adds output-object counts and degradation reasons for non-plain-text exports,
  making PDF display and JSON sidecars agree on what was rendered, referenced,
  or intentionally degraded.
